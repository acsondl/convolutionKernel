...
__constant__ float mask_c[16384];
...
for(int tile=0;tile<(numAColumns+TILE_WIDTH-1)/TILE_WIDTH;tile++){
    ...
    if(row<numARows&&numAColumns>aCol)A_s[threadIdx.y][threadIdx.x]=mask_c[(size_t)row*numAColumns+aCol];
...
cudaMemcpyToSymbol(mask_c,host_mask,(size_t)K*K*Channel*Map_out*sizeof(float));
