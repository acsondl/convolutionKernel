...
for(int tile=0;tile<(numAColumns+TILE_WIDTH-1)/TILE_WIDTH;tile++){
    ...
    if(col<numBColumns&&numBRows>bRow){
        ...
        sum+=A_s[threadIdx.y][0]*B_s[0][threadIdx.x];
        sum+=A_s[threadIdx.y][1]*B_s[1][threadIdx.x];
        sum+=A_s[threadIdx.y][2]*B_s[2][threadIdx.x];
        sum+=A_s[threadIdx.y][3]*B_s[3][threadIdx.x];
        sum+=A_s[threadIdx.y][4]*B_s[4][threadIdx.x];
        sum+=A_s[threadIdx.y][5]*B_s[5][threadIdx.x];
        sum+=A_s[threadIdx.y][6]*B_s[6][threadIdx.x];
        sum+=A_s[threadIdx.y][7]*B_s[7][threadIdx.x];
        sum+=A_s[threadIdx.y][8]*B_s[8][threadIdx.x];
        sum+=A_s[threadIdx.y][9]*B_s[9][threadIdx.x];
        sum+=A_s[threadIdx.y][10]*B_s[10][threadIdx.x];
        sum+=A_s[threadIdx.y][11]*B_s[11][threadIdx.x];
        sum+=A_s[threadIdx.y][12]*B_s[12][threadIdx.x];
        sum+=A_s[threadIdx.y][13]*B_s[13][threadIdx.x];
        sum+=A_s[threadIdx.y][14]*B_s[14][threadIdx.x];
        sum+=A_s[threadIdx.y][15]*B_s[15][threadIdx.x];
