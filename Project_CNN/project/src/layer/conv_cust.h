...
class Conv_Custom: public Layer {
 private:
  ...
  static constexpr float kCompetitionWarmupAbortThresholdMs = 40.0f;

  void init();
  float run_competition_forward(float* y, const float* x, const float* k,
                                int B, int M, int C, int K);
  static float compute_median(std::vector<float> values);

  static bool benchmark_enabled_;
  static int benchmark_warmup_rounds_;
  static int benchmark_measure_rounds_;
 ...
 public:
  ...
  static void configure_benchmark(bool enabled, int warmup_rounds = 5,
                                  int measure_rounds = 10);
